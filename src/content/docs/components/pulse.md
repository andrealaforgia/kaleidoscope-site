...
- **Series identity is the full label set (ADR-0045).** A series is identified by
...
- **Per-tenant cardinality watermark (ADR-0051).** Each tenant gets a soft cap of
...
- **Durability (ADR-0049, ADR-0059, ADR-0060).** WAL-plus-snapshot with real
...
ADR-0045 (series identity), ADR-0051 (cardinality watermark), ADR-0049 (honour
fsync), ADR-0059 (torn-tail recovery), ADR-0060 (store fsync durability); read
side: ADR-0042/0044/0046 (PromQL subset and label matchers), ADR-0050 (read
caps), ADR-0062 (raw points, `step` reserved).
