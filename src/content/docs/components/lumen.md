...
- **The read API (`log-query-api`).** `GET /api/v1/logs?start=&end=` returns a
 ...
 `min_severity`, `body_contains` (case-sensitive substring),
 `body_regex` (linear-time RE2-derived engine), and `limit`/`offset`
 pagination. An empty window is a calm `200 []`; a malformed window is
 a `400` before the store is touched.
- **Shared read caps.** A 24-hour window and a 100,000-row result cap,
