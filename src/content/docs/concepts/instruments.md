...
**Prism** is the query and visualisation frontend. At v0 it has three views: a
single-metric PromQL explorer, a trace explorer that finds failed traces and
shows their spans with correlated logs on one screen, and a logs search that
pivots from a symptom to its trace — the beginnings of a Grafana-style explore
view, with full dashboarding still to come. No seat licensing.
